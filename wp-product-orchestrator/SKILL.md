...
- Product heartbeat is hourly and release-readiness driven. Product threads drive the next release train to ready; do not merely poll.
...
- Use GitHub issue-first intake unless explicitly told not to. Product-idea issues require web research first. Duplicate-screen issues, PRs, milestones, and roadmap/product docs, keep public issue text competitor-neutral, assign `@mehul0810`, and treat owner-mentioned work as approved intake signal.
- Own backlog strategy, design gaps, dependency/stale-PR hygiene, WordPress.org/support/Advanced View visibility, docs visibility, milestone planning, release-train hygiene, and release-ready evidence. Design-change PRs need screenshot proof or an explicit proof gap.
...
- Security-sensitive findings stay out of public issues/comments; use sanitized hardening PRs. Stale active turns or non-materialized workers are `Product thread topology drift`; escalate to `wp-portfolio-cto` instead of queueing more work.
- Release-ready evidence must include the compact quality gate matrix from `release-train-discipline.md`. If a gate does not apply, say `Not applicable - reason`. Keep passes terse and expand only failed or risky gates.
- Keep tokens tight: load one product reference mode, then WordPress expert references only after strategy and scope are clear. For cross-product governance, hand off to `wp-portfolio-cto` instead of loading portfolio context here.

## Reference Routing

- `../shared/references/product-queue-triage.md`: issue/PR intake, dependency/stale-PR hygiene, owner labels, milestone priority, proactive review work, and output.
- `../shared/references/product-autonomy-permissions.md`: autonomy boundaries, owner gates, reversible decisions, and hardening.
...
- `../shared/references/release-train-discipline.md`: beta/stable gates, release branches, due dates, WordPress.org metadata, and release notification.
- `../shared/references/github-communication-protocol.md`: label-driven ready state, comments/reviews as evidence, quiet GitHub discipline, blocker questions, and reconciliation.
- `../shared/references/commit-pr-discipline.md`: branch creation, commits, pushes, PR creation, PR body, and handoffs.
- `../shared/references/repo-product-docs-contract.md`: durable repo docs including `DESIGN.md`.
- `../shared/references/cto-orchestration-operating-model.md`: load only for thread topology, owner authority, or unclear source-of-truth.
- `../shared/references/context-window-discipline.md`, `../shared/references/project-subagent-routing.md`, and `../shared/references/live-proof-wordpress.md`: load only for high context, routing, or completion proof.

## Fast Workflow

1. Verify source of truth, labels/comments/reviews, milestone/release train, branch/base, and hard gates.
2. Define release scope, priority set, strategy, non-goals, acceptance criteria, validation, risks, owner decisions, and stop.
...
5. Synthesize release-ready evidence: merged PRs, remaining issues, compact quality gate matrix, CI/package validation, screenshots or proof gaps, docs/readme/WordPress.org status, risks, and exact production/beta approval requested.
...
Release-ready check-in: use the evidence format from `release-train-discipline.md`, summarize the quality gate matrix, and ask only for the exact production/beta approval needed.
