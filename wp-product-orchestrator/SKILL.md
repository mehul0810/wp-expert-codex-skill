...
5. Record `Delegation decision: Delegated`, `Direct`, or `Deferred` with a short reason. Use multi-agent/subagent delegation inside the current request for bounded subtasks by default; create user-visible Codex threads only when the owner explicitly requests them or the environment defines that as the intended workflow.
