...
- Portfolio heartbeat/sweep: compact source-of-truth sweep; low or medium reasoning. The active acceleration default is every 30 minutes. Use high only for release conflict resolution, owner decision briefs, protected-thread recovery, or cross-product governance changes.
- Product hourly heartbeat or 15-minute acceleration heartbeat: medium reasoning by default. Use high only for release-ready synthesis, ambiguous milestone scope, risky PR merge/release decisions, security/privacy posture, schema/data migration, or product thread topology drift.
